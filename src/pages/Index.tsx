...
const Index = () => {
  ...
  return (
    <div className="p-8 space-y-12 bg-[#FFFBFA] min-h-screen">
      ...
      <div className="relative overflow-hidden rounded-[40px] bg-[#E8D5D8] p-10 aspect-[4/5] flex flex-col justify-between shadow-xl shadow-rose-100/50">
        <div className="absolute -top-20 -right-20 w-64 h-64 bg-white/20 rounded-full blur-3xl" />
        
        <div className="space-y-4 relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/30 backdrop-blur-md text-[10px] font-bold uppercase tracking-widest text-[#4A3F3F]">
            <Sparkles size={12} />
        ...
        <div className="space-y-4 relative z-10">
          <Button 
            ...
            className="w-full h-16 rounded-full bg-[#4A3F3F] text-white hover:bg-[#2D2424] text-lg font-bold shadow-lg"
          >
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <button onClick={() => navigate('/history')} className="text-[10px] font-bold text-[#8C7E7E] uppercase tracking-widest">History</button>
        </div>
        <div className="p-6 rounded-[32px] bg-[#F5F0E1] flex items-center gap-5 group cursor-pointer" onClick={() => navigate('/onboarding')}>
          <div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center text-[#4A3F3F] shadow-sm">
            <Wand2 size={24} />
          </div>
          <div className="flex-1">
            <p className="font-bold text-[#4A3F3F]">Change Event</p>
            <p className="text-xs text-[#8C7E7E]">Preparing for something new?</p>
          </div>
          <ChevronRight className="text-[#8C7E7E] group-hover:translate-x-1 transition-transform" />
        </div>
      </section>
    </div>
  );
